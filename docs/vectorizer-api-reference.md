...
| truncate   | boolean | true    | ✖        | Truncates the end of each input to fit within the chosen model's context length. Embedding fails if set to false and the context length is exceeded.    |
